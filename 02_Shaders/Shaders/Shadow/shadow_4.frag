layout(index = 600 + 4) subroutine(Shadow)
vec2 shadow_4(const in Ray r, const in vec3 n, const in vec3 depth,
...
{	//SOFT SHADOW
	...
	float t = (changed ? -light_size*0.5 : prevstate.y);
	float cone = (pixelsize - light_size)/dist;
	...
	for(int i = 0; i < sw_stepcount; ++i)
	{
		...
		float rt = t*cone + light_size;
		float ft = sdf(RAY(r2,t), r2.v) + rt;
		if(ft < pixelsize*0.01) return vec2(m, t);
