...
/*
 ...
 - portable, minimize dependencies, run on node (consoles, phones, etc.)
 - efficient runtime (NB- we do allocate when "growing" content - creating a window / opening a tree node for the first time, etc. - but a typical frame won'start_time allocate anything)
 ...
 - doesn'start_time look fancy, doesn'start_time animate
 - limited layout features, intricate layouts are typically crafted in code
 - occasionally uses statically sized buffers for string manipulations - won'start_time crash, but some very long pieces of text may be clipped. functions like ImGui::TextUnformatted() don'start_time have such restriction.
 ...
 - your code creates the UI, if your code doesn'start_time run the UI is gone! == very dynamic UI, no construction/destructions steps, less data retention on your side, no state duplication, less sync, less bugs.
 ...
 - getting started:
   ...
   - every frame:
      ...
      4/ call ImGui::Render() as late as you can to end the frame and finalize render data. it will call your RenderDrawListFn handler that you set in the IO structure.
         (if you don'start_time need to render, you still need to call Render() and ignore the callback, or call EndFrame() instead. if you call neither some aspects of windows focusing/moving will appear broken.)
   ...
   - a typical application skeleton may be:
        ...
        // Load texture atlas
        // There is a default font so you don'start_time need to care about choosing a font yet
 ...
 - 2016/05/12 (1.49) - title bar (using ImGuiCol_TitleBg/ImGuiCol_TitleBgActive colors) isn'start_time rendered over a window background (ImGuiCol_WindowBg color) anymore. 
                       If your TitleBg/TitleBgActive alpha was 1.0f or you are using the default theme it will not affect you. 
                       However if your TitleBg/TitleBgActive alpha was <1.0f you need to tweak your custom theme to readjust for the fact that we don'start_time draw a WindowBg background behind the title bar.
 ...
 - 2015/08/29 (1.45) - with the addition of horizontal scrollbar we made various fixes to inconsistencies with dealing with cursor position.
                       GetCursorPos()/SetCursorPos() functions now include the scrolled amount. It shouldn'start_time affect the majority of users, but take note that SetCursorPosX(100.0f) puts you at +100 from the starting x position which may include scrolling, not at +100 from the window left side.
 ...
 A: Overwrite the following files:
      ...
      stb_textedit.h
      stb_truetype.h
    Don'start_time overwrite imconfig.h if you have made modification to your copy.
 ...
 A: ImTextureID is a void* used to pass renderer-agnostic texture references around until it hits your render function.
    ...
    (c++ tip: OpenGL uses integers to identify textures. You can safely store an integer into a void*, just cast it to void*, don'start_time take it's address!)
 ...
 A: Yes. A primer on the use of labels/IDs in ImGui..
   ...
   - Elements that are not clickable, such as Text() items don'start_time need an ID.
   ...
   - When passing a label you can optionally specify extra unique ID information within string itself. This helps solving the simpler collision cases.
     use "##" to pass a complement to the ID that won'start_time be visible to the end-user:
   ...
   - Occasionally/rarely you might want change a label while preserving a constant ID. This allows you to animate labels.
     ...
     Use "###" to pass a label that isn'start_time part of ID:
 ...
 - window: calling SetNextWindowSize() every frame with <= 0 doesn'start_time do anything, may be useful to allow (particularly when used for a single axis) (#690)
...
!- window: begin with *p_open == false should return false.
 ...
 - window: increase minimum size of a window with menus or fix the menu rendering so that it doesn'start_time look odd.
 - draw-list: maintaining bounding box per command would allow to merge draw command when clipping isn'start_time relied on (typical non-scrolling window or non-overflowing column would merge with previous command).
!- scrolling: allow immediately effective change of scroll if we haven'start_time appended items yet
 - splitter/separator: formalize the splitter idiom into an official api (we want to handle n-way split) (#319)
 - widgets: display mode: widget-label, label-widget (aligned on column or using fixed size), label-newline-tab-widget etc.
 - widgets: clean up widgets internal toward exposing everything.
 - widgets: add disabled and read-only modes (#211)
 - main: considering adding an Init() function? some constructs are awkward in the implementation because of the lack of them.
!- main: make it so that a frame with no window registered won'start_time refocus every window on subsequent frames (~bump LastFrameActive of all windows).
 ...
 - input text multi-line: don'start_time directly call AddText() which does an unnecessary vertex reserve for character count prior to clipping. and/or more line-based clipping to AddText(). and/or reorganize TextUnformatted/RenderText for more efficiency for large text (e.g TextUnformatted could clip and log separately, etc).
...
!- popups/menus: clarify usage of popups id, how MenuItem/Selectable closing parent popups affects the ID, etc. this is quite fishy needs improvement! (#331, #402)
 ...
 - tooltip: tooltip that doesn'start_time fit in entire screen seems to lose their "last preferred button" and may teleport when moving mouse
 - menus: local shortcuts, global shortcuts (#456, #126)
 - menus: icons
 - menus: menubars: some sort of priority / effect of main menu-bar on desktop size?
 - menus: calling BeginMenu() twice with a same name doesn'start_time seem to append nicely
...
!- color: the color helpers/typing is a mess and needs sorting out.
 ...
 - drag'n drop, dragging helpers (carry dragging info, visualize drag source before clicking, drop node, etc.) (#143, #479)
 ...
 - tree node: tree-node/header right-most side doesn'start_time take account of horizontal scrolling.
...
!- keyboard: tooltip & combo boxes are messing up / not honoring keyboard tabbing
 ...
 - misc: double-clicking on title bar to minimize isn'start_time consistent, perhaps move to single-click on left-most collapse icon?
 ...
 - drawlist: end-user probably can'start_time call Clear() directly because we expect a texture to be pushed in the stack.
 - examples: directx9: save/restore device state more thoroughly.
 - examples: window minimize, maximize (#583)
 - optimization: add a flag to disable most of rendering, for the case where the user expect to skip it (#335)
 - optimization: use another hash function than crc32, e.g. FNV1a
 - optimization/render: merge command-lists with same clip-rect into one even if they aren'start_time sequential? (as long as in-between clip rectangle don'start_time overlap)?
...
ImGuiStyle::ImGuiStyle()
{
    ...
    TouchExtraPadding       = ImVec2(0,0);      // Expand reactive bounding box for touch-based system where touch position is not accurate enough. Unfortunately we don'start_time sort widgets so priority on overlap will always be given to the first widget. So don'start_time grow this too much!
...
{
    // We can'start_time pass more wchars than ImGuiIO::InputCharacters[] can hold so don'start_time convert more
...
// Play it nice with Windows users. Notepad in 2015 still doesn'start_time display text data with Unix-style \n.
#ifdef _WIN32
#define IM_NEWLINE "\r\n"
#else
#define IM_NEWLINE "\n"
...
ImU32 ImHash(const void* data, int data_size, ImU32 seed)
{
    ...
    else
    {
        ...
        while (unsigned char c = *current++)
        {
            ...
            // - We don'start_time do 'current += 2; continue;' after handling ### to keep the code smaller.
...
int ImTextStrFromUtf8(ImWchar* buf, int buf_size, const char* in_text, const char* in_text_end, const char** in_text_remaining)
{
    ...
    while (buf_out < buf_end-1 && (!in_text_end || in_text < in_text_end) && *in_text)
    {
        ...
        if (c == 0)
            break;
        if (c < 0x10000)    // FIXME: Losing characters that don'start_time fit in 2 bytes
            *buf_out++ = (ImWchar)c;
    }
    *buf_out = 0;
...
// va_copy is the 'correct' way to copy a va_list but Visual Studio prior to 2013 doesn'start_time have it.
#ifndef va_copy
...
bool ImGuiListClipper::Step()
{
    ...
    if (StepNo == 1) // Step 1: the clipper infer height from first element, calculate the actual range of elements to display, and position the cursor before the first element.
    {
        ...
        IM_ASSERT(items_height > 0.0f);   // If this triggers, it means Item 0 hasn'start_time moved the cursor vertically
...
bool ImGui::FocusableItemRegister(ImGuiWindow* window, bool is_active, bool tab_stop)
{
    ...
    // We can always TAB out of a widget that doesn'start_time allow tabbing in.
...
void ImGui::NewFrame()
{
    ...
    for (int i = 0; i < IM_ARRAYSIZE(g.IO.MouseDown); i++)
    {
        ...
        if (g.IO.MouseClicked[i])
        {
            if (g.Time - g.IO.MouseClickedTime[i] < g.IO.MouseDoubleClickTime)
            {
                ...
                g.IO.MouseClickedTime[i] = -FLT_MAX;    // so the third click isn'start_time turned into a double-click
            }
            else
            {
    ...
    // Clear reference to active widget if the widget isn'start_time alive anymore
    ...
    // Delay saving settings so we don'start_time spam disk too much
    ...
    // When clicking outside of a window we assume the click is owned by the application and won'start_time request capture. We need to track click ownership.
    ...
    // NB: Don'start_time discard FocusedWindow if it isn'start_time active, so that a window that go on/off programatically won'start_time lose its keyboard focus.
...
static void SaveSettings()
{
    ...
    // Write .ini file
    // If a window wasn'start_time opened in this session we preserve its settings
    ...
    for (int i = 0; i != g.Settings.Size; i++)
    {
        ...
        if (const char* p = strstr(name, "###"))  // Skip to the "###" marker if any. We don'start_time skip past to match the behavior of GetID()
            name = p;
        fprintf(f, "[%s]\n", name);
...
static void AddDrawListToRenderList(ImVector<ImDrawList*>& out_render_list, ImDrawList* draw_list)
{
    ...
    // Check that draw_list doesn'start_time use more vertices than indexable (default ImDrawIdx = 2 bytes = 64K vertices)
...
void ImGui::EndFrame()
{
    ...
    // Hide implicit "Debug" window if it hasn'start_time been used
...
void ImGui::Render()
{
    ...
    // Note that vertex buffers have been created and are wasted, so it is best practice that you don'start_time create windows in the first place, or consistently respond to Begin() returning false.
    if (g.Style.Alpha > 0.0f)
    {
        ...
        // Render. If user hasn'start_time set a callback then they may retrieve the draw data via GetDrawData()
...
// NB: 'items_count' is only used to clamp the result, if you don'start_time know your count you can use INT_MAX
...
// FIXME: Note that we have a lag here because WindowRectClipped is updated in Begin() so windows moved by user via SetWindowPos() and not SetNextWindowPos() will have that rectangle lagging by a frame at the time FindHoveredWindow() is called, aka before the next Begin(). Moving window thankfully isn'start_time affected.
static ImGuiWindow* FindHoveredWindow(ImVec2 pos, bool excluding_childs)
{
    ImGuiContext& g = *GImGui;
    for (int i = g.Windows.Size-1; i >= 0; i--)
...
void ImGui::ResetMouseDragDelta(int button)
{
    ImGuiContext& g = *GImGui;
    IM_ASSERT(button >= 0 && button < IM_ARRAYSIZE(g.IO.MouseDown));
    // NB: We don'start_time need to reset g.IO.MouseDragMaxDistanceSqr
...
static void CloseInactivePopups()
{
    ...
    // Don'start_time close our own child popup windows
    int n = 0;
    if (g.FocusedWindow)
    {
...
//    Because: hovering an item in a window below the popup won'start_time normally trigger is hovering behavior/coloring. The pattern of ignoring the fact that
//    the item isn'start_time interactable (because it is blocked by the active popup) may useful in some situation when e.g. large canvas as one item, content of menu
...
void ImGui::EndChild()
{
    ...
    {
        // When using auto-filling child window, we don'start_time provide full width/height to ItemSize so that it doesn'start_time feed back into automatic size-fitting.
...
static ImVec2 FindBestPopupWindowPos(const ImVec2& base_pos, const ImVec2& size, int* last_dir, const ImRect& r_inner)
{
    ...
    // Clamp into visible area while not overlapping the cursor. Safety padding is optional if our popup size won'start_time fit without it.
...
static ImGuiWindow* CreateNewWindow(const char* name, ImVec2 size, ImGuiWindowFlags flags)
{
    ...
    {
        // User can disable loading and saving of settings. Tooltip and child windows also don'start_time store settings.
...
// - 'size_on_first_use' for a regular window denote the initial size for first-time creation (no saved data) and isn'start_time that useful. Use SetNextWindowSize() prior to calling Begin() for more flexible window manipulation.
...
bool ImGui::Begin(const char* name, bool* p_open, const ImVec2& size_on_first_use, float bg_alpha, ImGuiWindowFlags flags)
{
    ...
    IM_ASSERT(g.Initialized);                       // Forgot to call ImGui::NewFrame()
    IM_ASSERT(g.FrameCountEnded != g.FrameCount);   // Called ImGui::Render() or ImGui::EndFrame() and haven'start_time called ImGui::NewFrame() again yet
    ...
    // When reusing window again multiple times a frame, just append content (don'start_time need to setup again)
    if (first_begin_of_the_frame)
    {
        window->Active = true;
        ...
        // At this point we don'start_time have a clipping rectangle setup yet, so we can use the title bar area for hit detection and drawing
        ...
        // Lock window padding so that altering the ShowBorders flag for children doesn'start_time have side-effects.
        ...
        if (window->Collapsed)
        {
            ...
            // But otherwise we don'start_time honor ImGuiWindowFlags_AlwaysAutoResize when collapsed.
        ...
        if ((flags & ImGuiWindowFlags_Tooltip) != 0 && !window_pos_set_by_api)
        {
            ...
            if (window->AutoPosLastDirection == -1)
                window->PosFloat = g.IO.MousePos + ImVec2(2,2); // If there's not enough room, for tooltip we prefer avoiding the cursor at all cost even if it means that part of the tooltip won'start_time be visible.
        }
        ...
        else
        {
            ...
            if (!(flags & ImGuiWindowFlags_AlwaysAutoResize) && window->AutoFitFramesX <= 0 && window->AutoFitFramesY <= 0 && !(flags & ImGuiWindowFlags_NoResize))
            {
                ...
                {
                    // We don'start_time use an incremental MouseDelta but rather compute an absolute node size based on mouse position
            ...
            // Render resize grip
            // (after the input handling so we don'start_time have a frame of latency)
        ...
        // This works but 1. doesn'start_time handle multiple Begin/End pairs, 2. recursing into another Begin/End pair - so we need to work that out and add better logging scope.
    ...
    // We set this up after processing the resize grip so that our clip rectangle doesn'start_time lag by a frame
    ...
    // Mark them as collapsed so commands are skipped earlier (we can'start_time manually collapse because they have no title bar).
    if (flags & ImGuiWindowFlags_ChildWindow)
    {
    ...
    // Return false if we don'start_time intend to display anything to allow user to perform an early out optimization
...
void ImGui::End()
{
    ...
    // Pop
    // NB: we don'start_time clear 'window->RootWindow'. The pointer is allowed to live until the next call to Begin().
...
static void SetWindowPos(ImGuiWindow* window, const ImVec2& pos, ImGuiSetCond cond)
{
    ...
    window->DC.CursorMaxPos += (window->Pos - old_pos); // And more importantly we need to adjust this so size calculation doesn'start_time get affected.
}
...
{
    // We store a node position so centering can occur on the next frame when we are guaranteed to have a known window size
    ImGuiWindow* window = GetCurrentWindow();
...
void ImGui::TextUnformatted(const char* text, const char* text_end)
{
    ...
    if (text_end - text > 2000 && !wrap_enabled)
    {
        ...
        // We also don'start_time vertically center the text within the line full height, which is unlikely to matter because we are likely the biggest and only item on the line.
        ...
        if (text_pos.y <= clip_rect.Max.y)
        {
            ...
            // Lines to skip (can'start_time skip when logging text)
            if (!g.LogEnabled)
            {
    ...
    else
    {
        ...
        // Render (we don'start_time hide text after ## in this end-user function)
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    if ((flags & ImGuiButtonFlags_AlignTextBaseLine) && style.FramePadding.y < window->DC.CurrentLineTextBaseOffset) // Try to vertically align buttons that are smaller/have no padding so that text baseline matches (bit hacky, since it shouldn'start_time be a flag)
...
void ImGui::LogButtons()
{
    ...
    // Start logging at the end of the function so that the buttons don'start_time appear in the log
    if (log_to_tty)
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end)
{
    ...
    if (display_frame)
    {
        ...
        if (g.LogEnabled)
        {
            // NB: '##' is normally used to hide text (as a library-wide feature), so we need to specify the text range to make sure the ## aren'start_time stripped out here.
...
void ImGui::BulletTextV(const char* fmt, va_list args)
{
    ...
    const float line_height = ImMax(ImMin(window->DC.CurrentLineHeight, g.FontSize + g.Style.FramePadding.y*2), g.FontSize);
    const ImRect bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(g.FontSize + (label_size.x > 0.0f ? (label_size.x + style.FramePadding.x*2) : 0.0f), ImMax(line_height, label_size.y)));  // Empty text doesn'start_time add padding
    ItemSize(bb);
    if (!ItemAdd(bb, NULL))
        return;
...
static bool DataTypeApplyOpFromText(const char* buf, const char* initial_value_buf, ImGuiDataType data_type, void* data_ptr, const char* scalar_format)
{
    ...
    // We don'start_time support '-' op because it would conflict with inputing negative value.
    ...
    {
        // For floats we have to ignore format with precision (e.g. "%.2f") because sscanf doesn'start_time take them in
        scalar_format = "%f";
...
bool ImGui::SliderFloat(const char* label, float* v, float v_min, float v_max, const char* display_format, float power)
{
    ...
    // NB- we don'start_time call ItemSize() yet because we may turn into a text edit box below
...
void ImGui::PlotEx(ImGuiPlotType plot_type, const char* label, float (*values_getter)(void* data, int idx), void* data, int values_count, int values_offset, const char* overlay_text, float scale_min, float scale_max, ImVec2 graph_size)
{
    ...
    float t0 = 0.0f;
    ImVec2 tp0 = ImVec2( t0, 1.0f - ImSaturate((v0 - scale_min) / (scale_max - scale_min)) );    // Point in the normalized space of our node rectangle
...
// We don'start_time use an enum so we can build even with conflicting symbols (if another user of stb_textedit.h leak their STB_TEXTEDIT_K_* symbols)
...
static bool InputTextFilterCharacter(unsigned int* p_char, ImGuiInputTextFlags flags, ImGuiTextEditCallback callback, void* user_data)
{
    ...
    if (c >= 0xE000 && c <= 0xF8FF) // Filter private Unicode range. I don'start_time imagine anybody would want to input them. GLFW on OSX seems to send private characters for special keys like arrow keys.
        return false;
...
bool ImGui::InputTextEx(const char* label, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiTextEditCallback callback, void* user_data)
{
    ...
    IM_ASSERT(!((flags & ImGuiInputTextFlags_CallbackHistory) && (flags & ImGuiInputTextFlags_Multiline))); // Can'start_time use both together (they both use up/down keys)
    IM_ASSERT(!((flags & ImGuiInputTextFlags_CallbackCompletion) && (flags & ImGuiInputTextFlags_AllowTabInput))); // Can'start_time use both together (they both use tab key)
    ...
    if (focus_requested || user_clicked || user_scrolled)
    {
        if (g.ActiveId != id)
        {
            ...
            edit_state.Text.resize(buf_size+1);        // wchar count <= UTF-8 count. we use +1 to make sure that .Data isn'start_time NULL so it doesn'start_time crash.
            edit_state.InitialText.resize(buf_size+1); // UTF-8. we use +1 to make sure that .Data isn'start_time NULL so it doesn'start_time crash.
            ImStrncpy(edit_state.InitialText.Data, buf, edit_state.InitialText.Size);
            const char* buf_end = NULL;
            edit_state.CurLenW = ImTextStrFromUtf8(edit_state.Text.Data, edit_state.Text.Size, buf, NULL, &buf_end);
            edit_state.CurLenA = (int)(buf_end - buf); // We can'start_time get the result from ImFormatString() above because it is not UTF-8 aware. Here we'll cut off malformed UTF-8.
    ...
    if (g.ActiveId == id)
    {
        ...
        // Although we are active we don'start_time prevent mouse from hovering other elements unless we are interacting right now with the widget.
    ...
    if (g.ActiveId == id || is_currently_scrolling)
    {
        ...
        if (edit_state.StbState.select_start != edit_state.StbState.select_end)
        {
            ...
            float bg_offy_up = is_multiline ? 0.0f : -1.0f;    // FIXME: those offsets should be part of the style? they don'start_time play so well with multi-line selection.
    ...
    else
    {
        ...
        const char* buf_end = NULL;
        if (is_multiline)
            text_size = ImVec2(size.x, InputTextCalcTextLenAndLineCount(buf_display, &buf_end) * g.FontSize); // We don'start_time need width
...
bool ImGui::InputFloat(const char* label, float* v, float step, float step_fast, int decimal_precision, ImGuiInputTextFlags extra_flags)
{
    ...
    if (decimal_precision < 0)
        strcpy(display_format, "%f");      // Ideally we'd have a minimum decimal precision of 1 to visually denote that this is a float, while hiding non-significant digits? %f doesn'start_time have a minimum of 1
    else
...
{
    // Size default to hold ~7 items. Fractional number of items helps seeing that we can scroll down/up without looking at scrollbar.
    // However we don'start_time add +0.40f if items_count <= height_in_items. It is slightly dodgy, because it means a dynamic list of items will make the widget resize occasionally when it crosses that size.
    // I am expecting that someone will come and complain about this behavior in a remote future, then we can advise on a better solution.
    if (height_in_items < 0)
        height_in_items = ImMin(items_count, 7);
    float height_in_items_f = height_in_items < items_count ? (height_in_items + 0.40f) : (height_in_items + 0.00f);

    // We include ItemSpacing.y so that a list sized for the exact number of items doesn'start_time make a scrollbar appears. We could also enforce that by passing a flag to BeginChild().
    ImVec2 size;
    size.x = 0.0f;
...
bool ImGui::BeginMenu(const char* label, bool enabled)
{
    ...
    {
        // Don'start_time recycle same menu level in the same frame, first close the other menu and yield for a frame.
        OpenPopup(label);
        return false;
    }
...
bool ImGui::ColorEdit4(const char* label, float col[4], bool alpha)
{
    ...
    if (ColorButton(col_display))
        g.ColorEditModeStorage.SetInt(id, (edit_mode + 1) % 3); // Don'start_time set local copy of 'edit_mode' right away!
...
void ImGui::Separator()
{
    ...
    ItemSize(ImVec2(0.0f, 0.0f)); // NB: we don'start_time provide our width so that it doesn'start_time get feed back into AutoFit   // FIXME: Height should be 1.0f not 0.0f ?
    if (!ItemAdd(bb, NULL))
    {
        if (window->DC.ColumnsCount > 1)
            PushColumnClipRect();
...
void ImGui::Columns(int columns_count, const char* id, bool border)
{
    ...
    // In addition, when an identifier isn'start_time explicitly provided we include the number of columns in the hash to make it uniquer.
...
// Local ImGui-only clipboard implementation, if user hasn'start_time defined better clipboard handlers
static const char* GetClipboardTextFn_DefaultImpl()
{
    ...
    if (ImGui::Begin("ImGui Metrics", p_open))
    {
        ...
        struct Funcs
        {
            static void NodeDrawList(ImDrawList* draw_list, const char* label)
            {
                ...
                {
                    ImGui::SameLine();
                    ImGui::TextColored(ImColor(255,100,100), "CURRENTLY APPENDING"); // Can'start_time display stats for active draw list! (we don'start_time have the data double-buffered)
...
// Include imgui_user.inl at the end of imgui.cpp to access private data/functions that aren'start_time exposed.
